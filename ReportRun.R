...
unfroz <- transform(unfroz,acc_pct_chg_since_tracing=percent(acc_pct_chg_since_tracing),
                    stockName=stockID2name(stockID),
                    periodrtn_index=percent(periodrtn_index))
unfroz <- left_join(unfroz,tmpindexname[,1:2],by='indexID')
unfroz <- unfroz[,c("stockID","stockName","tracing_start_date","unfroz_date"              
                    ,"acc_pct_chg_since_tracing","indexName","periodrtn_index")]
colnames(unfroz) <- c('股票代码','股票简称','跟踪起始日','解禁日','区间涨跌幅','所属行业指数','行业指数涨跌幅')




########################### ~~ index 000985E ~~ #################################
lcdb.update.QT_DailyQuote()
...
########################### ~~  super small stock ~~ #################################
load("E:/Users/aming.tao/Documents/R/Report/smallStock.RData")
